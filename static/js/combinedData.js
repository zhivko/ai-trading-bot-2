    ...
    combinedWebSocket.onmessage = (event) => {
        try {
            ...
            if (message.type === 'historical') {
                //console.log('Combined WebSocket: Processing historical message with', message.data ? message.data.length : 0, 'data points');
                handleHistoricalData(message);
            } else if (message.type === 'live') {
                //console.log('Combined WebSocket: Processing live message');
                handleLiveData(message);
            } else {
                console.warn('Combined WebSocket: Unknown message type:', message.type);
            }
        } catch (e) {
...
function updateChartWithHistoricalData(dataPoints, symbol) {
    ...
    if (!dataPoints || dataPoints.length === 0) {
        console.warn('Combined WebSocket: No historical data points to process');
        return;
    }
    ...
    try {
        ...
        // Use the original layout which preserves user's axis ranges
...
function createLayoutForIndicators(activeIndicatorIds) {
    ...
    if (activeIndicatorIds.length > 0) {
        // Use the working approach from old.py - unit-based calculation with gaps
        const totalUnits = 3 + activeIndicatorIds.length; // Price gets 3 units, each indicator gets 1 unit
        const unitHeight = 1.0 / totalUnits; // Each unit's height

        const priceChartHeight = 3 * unitHeight; // Price chart gets 3 units (increased from 2)
        const labelGap = 0.04; // Larger gap for labels (from working old.py version)
        const availableForIndicators = 1.0 - priceChartHeight - labelGap;
        const gapBetweenIndicators = 0.005; // 0.5% gap between indicators
        ...
        activeIndicatorIds.forEach((indicatorId, index) => {
            ...
            let domainStart = indicatorsStartAt - (indicatorIndex + 1) * indicatorHeight - indicatorIndex * gapBetweenIndicators;
            let domainEnd = domainStart + indicatorHeight;

            // Ensure domain doesn't go below 0
            domainStart = Math.max(0, domainStart);
            domainEnd = Math.min(indicatorsStartAt, domainEnd);
